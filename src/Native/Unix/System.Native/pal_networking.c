...
int32_t SystemNative_Bind(intptr_t socket, uint8_t* socketAddress, int32_t socketAddressLen)
{
    ...
    intptr_t socket, int32_t socketOptionLevel, int32_t socketOptionName, uint8_t* optionValue, int32_t* optionLen)
{
    ...
    if (socketOptionLevel == SocketOptionLevel_SOL_SOCKET)
    {
        ...
        else if (socketOptionName == SocketOptionName_SO_REUSEADDR)
        {
            //
            ...
            if (err == 0 && *(uint32_t*)optionValue != 0)
            {
                err = SystemNative_ConvertErrorPlatformToPal(errno);
            }
...
SystemNative_SetSockOpt(intptr_t socket, int32_t socketOptionLevel, int32_t socketOptionName, uint8_t* optionValue, int32_t optionLen)
{
    ...
    if (socketOptionLevel == SocketOptionLevel_SOL_SOCKET)
    {
        ...
        else if (socketOptionName == SocketOptionName_SO_REUSEADDR)
        {
            ...
            //
            int err = setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, optionValue, (socklen_t)optionLen);
            if (err == 0)
            {
                err = setsockopt(fd, SOL_SOCKET, SO_REUSEPORT, optionValue, (socklen_t)optionLen);
            }
            return err == 0 ? Error_SUCCESS : SystemNative_ConvertErrorPlatformToPal(errno);
        }
    }
